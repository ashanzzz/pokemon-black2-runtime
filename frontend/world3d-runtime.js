...
export class Black2World3D{
  ...
  async loadStaticZone(zoneId){
    ...
    const req=(async()=>{this.staticPreview=true;this.staticPreviewZone=id;this.ui.onStatus?.('player','candidate',`static preview Zone ${id}`);this.ui.onStatus?.('scene','candidate',`loading static Zone ${id}`);try{
      const raw=await getJSON(`${API}/scene/zone/${encodeURIComponent(id)}`,10000),data=this._staticPreviewEnvelope(raw,id),key=data.render_key||data.scene_key;
  ...
  _clipTerrainToCell(root,item,origin,span){
    ...
    root.traverse(o=>{
      ...
      const clipped=source.map(m=>{const copy=m.clone();copy.clippingPlanes=planes;copy.clipIntersection=false;copy.needsUpdate=true;return copy});
