    ...
    def __init__(self):
        self.capabilities: Dict[str, CapabilityDetail] = {
            ...
            "trainer_profile": CapabilityDetail(
                ...
                can_detect=[],
                missing=[
                    "Verified Trainer/SaveBlock runtime locator",
                    "Player name, gender, badges, money, and play-time field offsets",
                    "Cross-save and cross-session validation",
                ],
                known_limitations="Raw Main RAM/SRAM dumps exist, but no verified trainer-profile decoder is wired into the runtime.",
                source="Raw RAM/SRAM evidence only",
                validator="none",
            ),
            "party_pokemon": CapabilityDetail(
                ...
                can_detect=[],
                missing=[
                    "Verified party root and count address",
                    "Six-slot base, stride, and inactive-slot rules",
                    "Gen 5 encrypted-block permutation and checksum validation",
                    "Controlled cross-snapshot validation of species, level, HP, held item, and moves",
                ],
                known_limitations="The party module defines response models only; no verified RAM locator or decoder exists.",
                source="Raw RAM evidence only",
                validator="none",
            ),
            "battle_system": CapabilityDetail(
                ...
                can_detect=[],
                missing=[
                    "Verified battle-active locator",
                    "Battle kind, phase, menu, cursor, and legal-action decoder",
                    "Party, move, item, and target mapping",
                    "Closed-loop action execution and result validation",
                ],
                known_limitations="No verified battle RAM decoder is wired into the runtime; battle inactivity cannot be inferred.",
                source="Raw RAM evidence only",
                validator="none",
            ),
