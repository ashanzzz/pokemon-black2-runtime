...
async def post_nav_find_path(req: FindPathRequest):
    ...
    return {
        ...
        "confidence": "candidate_static_2d",
        "warning": "Legacy flat permission A* cannot represent bridge/floor elevation. Use /api/v1/lab/navigation/path for observed layered routes.",
    }
...
async def post_nav_navigate_to(req: NavigateToRequest):
    ...
    if os.getenv("BLACK2_ENABLE_LEGACY_NAVIGATION", "0").strip().lower() not in {"1", "true", "yes", "on"}:
        raise HTTPException(status_code=409, detail={
            "reason": "legacy flat 2D navigation is disabled by default",
            "risk": "bridge/stairs/floor elevation semantics are not verified",
            "use": "/api/v1/lab/navigation/path",
            "override": "set BLACK2_ENABLE_LEGACY_NAVIGATION=1 only for controlled experiments",
        })
    st = await state_engine.sample_once()
    pos = st.player_world_pos or {}
    sx = pos.get("x")
